...
getSummaryInf<-function(rdat, #rdat #rdat<-Stratify(dat)   i.e. stratified individual dataset
  ...
  if( family_used == 'coxph' ){
    if(  is.null(rdat$time)|is.null(rdat$event)){ stop('when using coxph, data must have the columns named as: time and event')}
  }
  ...
  if(covariate){
    cat(  'the covariates to adjust are:', colnames(rdat)[cov_pos], '(empty means no covariate to adjust)' , '\n' )
  }
    ...
    for(i in 1:Ns){
      ...
      if( length(table(selected_dat$Z))==1  ){#如果Z都是单值的，谈何IV analysis?
        ...
        if(!covariate){ #no covariate adjustment
          fitGX<-lm(    selected_dat$X~  selected_dat$Z  );bGX<-summary(fitGX)$coef[-1,1]
          seGX<- summary(fitGX)$coef[-1,2] ; bx<-as.numeric( bGX  ); bxse<-as.numeric(  seGX)
          ...
          if(family_used=='coxph'){
            fitGY <- coxph( Surv(selected_dat$time, selected_dat$event) ~  selected_dat$Z    )
          ...
          fitGX<-lm(    selected_dat$X~  selected_dat$Z + as.matrix( selected_dat[,cov_pos] )  )
          bGX<-summary(fitGX)$coef[2,1] ;seGX<- summary(fitGX)$coef[2,2] ; bx<-as.numeric( bGX  ); bxse<-as.numeric(  seGX)
          Fvalue<- summary(fitGX)$coef[2,3]^2    #IV strength - F value
          if(family_used!='coxph'){
          ...
          if(family_used=='coxph'){
            fitGY <- coxph( Surv(selected_dat$time, selected_dat$event) ~  selected_dat$Z +  as.matrix( selected_dat[,cov_pos] )   )
